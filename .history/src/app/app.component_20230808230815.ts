...
export class AppComponent implements OnInit {
  ...
  private handleEvent(eventType: string, event: Event) {
    ...
    if (eventType === 'click' && target.tagName === 'BUTTON') {
      keypress = target.textContent || target.innerText || '';
    ...
    const logEntry = { userId, action, timestamp, module, keypress };

    // Logging the keypresss to the console
    console.log(`User: ${userId}, Module: ${module}, Action: ${action}, keypress: ${keypress}, Timestamp: ${timestamp}`);
